...
def gen_ecdsa_p256(keyfilename):
    key = keys.ECDSA256P1.generate()
    key.export_private(keyfilename)
    key.export_public("PUB_"+keyfilename)

def gen_ecdsa_p384(keyfilename):
    key = keys.ECDSA384P1.generate()
    key.export_private(keyfilename)
    key.export_public("PUB_"+keyfilename)
	
def gen_aes_gcm(keyfilename):
    keys.AES_GCM.generate().export_private(keyfilename)

def gen_aes_cbc(keyfilename):
    keys.AES_CBC.generate().export_private(keyfilename)

def gen_aes256_cbc(keyfilename):
    keys.AES256_CBC.generate().export_private(keyfilename)

def gen_aes_ctr(keyfilename):
    keys.AES_CTR.generate().export_private(keyfilename)


keygens = {
        'aes-gcm': gen_aes_gcm,
        'aes-cbc': gen_aes_cbc,
        'aes256-cbc': gen_aes256_cbc,
        'aes-ctr': gen_aes_ctr,
...
def do_keygen(args):
    ...
    if args.num == 1:
        keygens[args.type](args.key)
    elif args.num > 8:
        print("Max number of keys supported is 8!")
    else:
        for i in range (0, args.num):
            print("Generate key index " + str(i) + ": Private key file name: ["+args.key+"_"+str(i)+"]")
            keygens[args.type](args.key+"_"+str(i))
...
def do_trans(args):
    if args.num == 1 :
        ...
        for idx in range(0, args.num):
            key = keys.load(args.key+"_"+str(idx))
            end = False
            if args.end:
                end = True
            if args.assembly=="ARM" or args.assembly=="IAR" or args.assembly=="GNU":
                if args.version=="V6M" or args.version=="V7M": 
                    out = key.trans(args.section, args.function+"_"+str(idx), end, args.assembly, args.version)
                    print (str(out))
                else:
                    print ("-v option : Cortex M architecture not supported")
                    exit(1)
...
def do_sign(args):
    ...
    with open(args.infile, 'rb') as f:
        payload = f.read()
    # Removing the padding because the hashlib pads properly (512 bit alignment) for SHA256/384
    # payload = pad(payload)
...
def do_encrypt(args):
    ...
    # Here we need to do different things if args.keynum > 1 for key revocation support case
    if args.keynum == 1:
        # we consider that there is only one file 
        key = keys.load(args.key) if args.key else None
        print("Key file for image singing is "+args.key)
    else:
        print("total number of keys in key set: "+str(args.keynum))
        print("encryption key index: "+str(args.enckeyidx))
        if args.enckeyidx >= args.keynum:
            print("Encryption key index must be smaller than keynum")
            ex(1)
        else:        
            print("Key file for image encryption is "+args.key+"_"+str(args.enckeyidx))
            key = keys.load(args.key+"_"+str(args.enckeyidx)) if args.key else None        
...
def do_header_lib(args):
    ...
    if args.keynum == 1:
        ...
        print("total number of keys in key set: "+str(args.keynum))
        print("signing key index: "+str(args.signkeyidx))
        if args.signkeyidx >= args.keynum:
            print("Signing key index must be smaller than keynum")
            ex(1)
        elif args.enckeyidx >= args.keynum:
            print("Encryption key index must be smaller than keynum")
            ex(1)
        else:
            print("Key file for image singing is "+args.key+"_"+str(args.signkeyidx))
            key = keys.load(args.key+"_"+str(args.signkeyidx)) if args.key else None        
    #empty nonce
    nonce = b''
    protocol = args.protocol
    magic = args.magic.encode()
    version = args.version    
    signkeyidx = args.signkeyidx
    enckeyidx = args.enckeyidx
    ...
    header = pack('<'+str(len(magic))+'sHHIII'+str(len(tag))+'s'+str(len(pfwtag))+'s'+str(len(nonce))+'sBB'+str(args.reserved)+'s',
                    magic, protocol, version, size, pfwoffset, pfwsize, tag, pfwtag, nonce, enckeyidx, signkeyidx, reserved)
    #add certs
    # Add certificates
    if args.cert_fw_leaf :
...
def do_header(args):
    ...
    if len(signature) == 16:
        ...
        signature = signature[0:96] #FIXME
    else:
...
subcmds = {
        ...
        'sha256': do_sha256,
        #hash a file with sha384 and provide result in in a file
        'sha384': do_sha384,
...
def args():
    ...
    keygenp.add_argument('-n', '--num', type=int, default=1)
    keygenp.add_argument('-t', '--type', metavar='type',
            choices=['aes-gcm', 'ecdsa-p256','ecdsa-p384','aes-cbc','aes256-cbc','aes-ctr'],
            required=True)
    ...
    trans.add_argument('-n', '--num', type=int, default=1)
    ...
    sha256 = subs.add_parser('sha256', help='hash a file with sha256')
    sha256.add_argument("infile")
    sha256.add_argument("outfile")
    sha256.add_argument('-p', '--padding',type=int, required = False, default=0, help='pad to be a multiple of the given size if needed')
    sha384 = subs.add_parser('sha384', help='hash a file with sha384')
    sha384.add_argument("infile")
    sha384.add_argument("outfile")
    sha384.add_argument('-p', '--padding',type=int, required = False, default=0, help='pad to be a multiple of the given size if needed')
    ...
    enc.add_argument('--enckeyidx', metavar='enckeyidx', help='index of encryption key, 0 by default', type=int, default=0)
    enc.add_argument('--keynum', metavar='keynum', help='number of keys in key set. 1 by default. If key revocation is supported, max is 8', type=int, default=1)
    ...
    head.add_argument('--signkeyidx', metavar='signkeyidx', help='index of signing key, 0 by default', type=int, default=0)
    head.add_argument('--enckeyidx', metavar='enckeyidx', help='index of encryption key, 0 by default', type=int, default=0)    
    head.add_argument('--keynum', metavar='keynum', help='number of keys in key set. 1 by default. If key revocation is supported, max is 8', type=int, default=1)
    ...
    pack.add_argument('--signkeyidx', metavar='signkeyidx', help='index of signing key, 0 by default', type=int, default=0)
    pack.add_argument('--enckeyidx', metavar='enckeyidx', help='index of encryption key, 0 by default', type=int, default=0)    
    pack.add_argument('--keynum', metavar='keynum', help='number of keys in key set. 1 by default. If key revocation is supported, max is 8', type=int, default=1)
