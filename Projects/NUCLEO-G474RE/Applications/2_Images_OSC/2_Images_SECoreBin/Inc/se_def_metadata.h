...
#define SE_SYMKEY_LEN           (32U)  /*!< SE Symmetric Key length (bytes)*/
/* AES-CBC encryption (or no encryption) and SHA256 for FW tag */
#define SE_IV_LEN               (16U)  /*!< Secure Engine IV Length (Bytes): same size as an AES block*/
#define SE_TAG_LEN              (48U)  /*!< Secure Engine Tag Length (Bytes): SHA-384 for the FW tag */
#define SE_HEADER_SIGN_LEN      (96U)  /*!< Firmware Header signarure LEN*/
#define SE_ASYM_PUBKEY_LEN      (96U)  /*!< SE Asymmetric Public Key length (bytes)*/
...
typedef struct
{
  ...
  uint8_t  EncKeyIndex;            /*!< Index of the key used for FW encryption. In case key revocation not supported, this field is always 0 */
  uint8_t  PubKeyIndex;            /*!< Index of the pub key used for FW verification. In case key revocation not supported, this field is always 0 */
  uint8_t  Reserved[110U];         /*!< Reserved for future use: 110 extra bytes to have a header size of 192 bytes */
...
typedef struct
{
  ...
  uint8_t  EncKeyIndex;            /*!< Index of the key used for FW encryption. In case key revocation not supported, this field is always 0 */
  uint8_t  PubKeyIndex;            /*!< Index of the pub key used for FW verification. In case key revocation not supported, this field is always 0 */
  uint8_t  Reserved[26U];          /*!< Reserved for future use: 26 extra bytes to have a header size of 192 bytes */
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
...
typedef struct
{
  ...
  uint8_t  EncKeyIndex;            /*!< Index of the key used for FW encryption. In case key revocation not supported, this field is always 0 */
  uint8_t  PubKeyIndex;            /*!< Index of the pub key used for FW verification. In case key revocation not supported, this field is always 0 */
  uint8_t  Reserved[2U];          /*!< Reserved for future use: 2 extra bytes to have a header size of 232 bytes ==> To make the header size aligned to flash write size (8bytes) */
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
...
typedef struct
{
  ...
  uint8_t  EncKeyIndex;            /*!< Index of the key used for FW encryption. In case key revocation not supported, this field is always 0 */
  uint8_t  PubKeyIndex;            /*!< Index of the pub key used for FW verification. In case key revocation not supported, this field is always 0 */
  uint8_t  Reserved[42U];          /*!< Reserved for future use: 42 extra bytes to have a header size of 192 bytes */
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
...
typedef struct
{
  ...
  uint8_t  EncKeyIndex;            /*!< Index of the key used for FW encryption. In case key revocation not supported, this field is always 0 */
  uint8_t  PubKeyIndex;            /*!< Index of the pub key used for FW verification. In case key revocation not supported, this field is always 0 */
  uint8_t  Reserved[18U];          /*!< Reserved for future use: 18 extra bytes to have a header size of 232 bytes ==> To make the header size aligned to flash write size (8bytes)*/
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
