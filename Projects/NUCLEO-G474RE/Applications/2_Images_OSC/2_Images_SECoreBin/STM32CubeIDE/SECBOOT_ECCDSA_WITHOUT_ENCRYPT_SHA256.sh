...
ret=$?
if [ $ret -eq 0 ]; then 
  command=$cmd" "$prepareimage" pack -m "$magic" -k "$ecckey" -p 1 -r 42 -v "$version" -f "$bin" -t "$sign" "$sfb" -o "$offset 
  $command >> $projectdir"/output.txt"
  ret=$?
  if [ $ret -eq 0 ]; then
    command=$cmd" "$prepareimage" header -m "$magic" -k  "$ecckey"  -p 1 -r 42 -v "$version"  -f "$bin" -t "$sign" -o "$offset" "$headerbin
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret -eq 0 ]; then
      ...
      if [ $ret -eq 0 ] && [ -e "$ref_userapp" ]; then
        ...
        if [ $ret -eq 0 ]; then
          ...
          ret=$?
          if [ $ret -eq 0 ]; then
            command=$cmd" "$prepareimage" pack -m "$magic" -k "$ecckey" -p 1 -r 42 -v "$version" -f "$bin" -t "$sign" -o "$offset" --pfw "$partialbin" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
            $command >> $projectdir"/output.txt"
            ret=$?
          fi
        fi
