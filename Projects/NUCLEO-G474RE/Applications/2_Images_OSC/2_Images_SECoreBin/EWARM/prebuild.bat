...
:: number of keys can be given in the 2nd parameter
:: if the 2nd parameter is not given, by default single key set is used (no key revocation support in this case)
if "%2"=="" (
    set "keynum=1"
) else (
	set "keynum=%2"
...
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey_1 -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY2_key_AES_%type%.bin"
IF NOT EXIST %oemkey% goto AES128_end
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey_2 -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY3_key_AES_%type%.bin"
IF NOT EXIST %oemkey% goto AES128_end
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey_3 -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error
...
:ECDSA
set "ecckey=%1\\..\\Binary\\%pubprefix%ECCKEY1.txt"
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_1_Pub -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "ecckey=%1\\..\\Binary\\%pubprefix%ECCKEY2.txt"
IF NOT EXIST %ecckey% goto end
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_2_Pub -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "ecckey=%1\\..\\Binary\\%pubprefix%ECCKEY3.txt"
IF NOT EXIST %ecckey% goto end
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_3_Pub -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error
...
:AES256
set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY1_key_AES256_%type%.bin"
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey_1 -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY2_key_AES256_%type%.bin"
IF NOT EXIST %oemkey% goto AES256_end
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey_2 -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY3_key_AES256_%type%.bin"
IF NOT EXIST %oemkey% goto AES256_end
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey_3 -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error
...
:ECDSA384
set "ecckey=%1\\..\\Binary\\%pubprefix%ECCKEY1-384.txt"
echo key file to be translated %ecckey%  >> %1\\output.txt 2>&1
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_1_Pub -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "ecckey=%1\\..\\Binary\\%pubprefix%ECCKEY2-384.txt"
IF NOT EXIST %ecckey% goto end
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_2_Pub -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "ecckey=%1\\..\\Binary\\%pubprefix%ECCKEY3-384.txt"
IF NOT EXIST %ecckey% goto end
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_3_Pub -v %cortex% -n %keynum% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error
