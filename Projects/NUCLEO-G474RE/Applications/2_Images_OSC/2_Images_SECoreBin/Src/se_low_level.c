/**
  ...
  ******************************************************************************
  */
#if !defined SBSFU_SINGLE_PROJECT
...
/**
  * @brief  OTP write function.
  * @param  wordIdx: Index of word in OTP area (starting from 0)
  * @param  pInData: pointer to the input data buffer
                     size of input buffer shall be nbWords x 8 bytes
  * @param  nbWords: number of words to program (1 word = 8 bytes)
  * @retval SE_ErrorStatus SE_SUCCESS if successful, SE_ERROR otherwise
  */    
SE_ErrorStatus SE_LL_FLASH_OTP_Write(uint8_t wordIdx, uint8_t *pInData, int nbWords)
{
  SE_ErrorStatus ret = SE_SUCCESS;
  uint32_t i;
  uint32_t destination = (uint32_t)(OTP_BASE_ADDR + wordIdx*OTP_WORD_SIZE_IN_BYTES);
  uint32_t source = (uint32_t)pInData;

  if ((nbWords == 0U) || ((wordIdx+nbWords) > OTP_WORD_IDX_MAX))
  {
    return SE_ERROR;
  }
  ...
  {
    for (i = 0U; i < nbWords; i += OTP_WORD_SIZE_IN_BYTES)
    {
...
/**
  ...
  * @param  wordIdx: Index of word in OTP area (starting from 0)
  * @param  pOutData: pointer to the output data buffer to hold the read data.
                      size of output buffer shall be nbWords x 8 bytes
  * @param  nbWords: number of words to read (1 word = 8 bytes).
  * @retval SE_ErrorStatus SE_SUCCESS if successful, SE_ERROR otherwise
  */
SE_ErrorStatus SE_LL_FLASH_OTP_Read(uint8_t wordIdx, uint8_t *pOutData, int nbWords)
{
  ...
  Length = nbWords * OTP_WORD_SIZE_IN_BYTES;
  pSource = (uint8_t*)(OTP_BASE_ADDR + wordIdx * OTP_WORD_SIZE_IN_BYTES);
  
  SE_DoubleECC_Error_Counter = 0U;
  /* Do not use memcpy from lib : ECC error should be checked at each loop */
  for (i = 0; (i < Length) && (SE_DoubleECC_Error_Counter == 0U); i++, pOutData++, pSource++)
  {
    *pOutData = *pSource;
  }
