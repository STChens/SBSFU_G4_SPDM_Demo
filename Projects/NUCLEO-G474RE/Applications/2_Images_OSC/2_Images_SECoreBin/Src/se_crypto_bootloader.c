...
/** @defgroup SE_CRYPTO_BOOTLOADER_Private_Variables Private Variables
  ...
  */
    ...
    || (SECBOOT_CRYPTO_SCHEME == SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM) )
...
#if ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384))
...
/** @defgroup SE_CRYPTO_BOOTLOADER_Private_Variables_SHA256 SHA256 variables
  ...
  *  @{
  */
static mbedtls_sha256_context m_SHA256ctx; /*!< Variable used to store the SHA256 context (this one does not really need to be
                                       protected as it is a HASH) */

#elif ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384) )
/** @defgroup SE_CRYPTO_BOOTLOADER_Private_Variables_SHA256 SHA256 variables
  *  @brief  Secure Hash Algorithm SHA-256.
  *  @note   We do not use local variable(s) because the context is used to store internal states.
  *  @{
  */
static mbedtls_sha512_context m_SHA512ctx; /*!< Variable used to store the SHA256 context (this one does not really need to be
                                       protected as it is a HASH) */
/**
  * @}
  */
...
/**
  ...
  */
#if ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) \
  || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384))
...
/**
  * @brief  Validate key index from header vs. stored min key index value
  * @param  pxSE_Metadata: Firmware metadata.
  * @param  keyflag: 
            bit 0 == 1: check for public key
            bit 1 == 1: check for encryption key
  * @param  updateOTP: 
            1: update OTP value if index from header is bigger than stored min index
            0: Only check, no OTP update
  * @retval 1: validate succeed, otherwise, validate failed
  */
static int ValidateKeyIndex(SE_FwRawHeaderTypeDef *pxSE_Metadata, uint8_t keyflag, int updateOTP)
{
  int ret = -1;
  
  /* First check public key flag */ 
  if ( (keyflag & 0x1) == 0x1 )
  {
    /* Check Public key index validity */
    uint8_t pubkeyidx = pxSE_Metadata->PubKeyIndex;
    uint8_t pubkeyidx_min = 0;
    int i;
    
    if(pubkeyidx > KEY_IDX_MAX ) return -1;
    
    for(i = 0; i < KEY_IDX_MAX ; i++)
    {
      uint32_t idxVal[2] = {0,0};
      SE_LL_FLASH_OTP_Read((OTP_PUB_KEY_IDX_WORD_START+i), (uint8_t*)&idxVal[0], 1);
      if ( (idxVal[0] != 0xFFFFFFFF) || (idxVal[1] != 0xFFFFFFFF))
      {
        /* we have found the key index min value from OTP */
        pubkeyidx_min = i+1;      
      }
    }
#ifdef TEST_KEY_REVOCATION_STORAGE_IN_BKPR     
    if ( pubkeyidx_min == 0 )
    {
      pubkeyidx_min = (uint8_t)(TAMP->BKP0R & 0xFF);
    }
#endif    
    if (pubkeyidx >= pubkeyidx_min)
    {
      if ( (pubkeyidx > pubkeyidx_min) && (updateOTP == 1) )
      {
        /* Program OTP to set the new MIN INDEX FLAG */
#ifdef TEST_KEY_REVOCATION_STORAGE_IN_BKPR         
        // For test purpose, we store the index in a RAM value first
        TAMP->BKP0R = pubkeyidx;
#else 
        uint32_t idxVal[2] = {0,0};
        SE_LL_FLASH_OTP_Write((OTP_PUB_KEY_IDX_WORD_START + pubkeyidx - 1), (uint8_t *)&idxVal[0], 1);
#endif          
      }
      ret = 1;
    }
  ...
  if ( (keyflag & 0x2) == 0x2 )
  {
    /* Check Encryption key index validity */
    uint8_t enckeyidx = pxSE_Metadata->EncKeyIndex;
    uint8_t enckeyidx_min = 0;
    int i;
    
    if(enckeyidx > KEY_IDX_MAX ) return -1;
    
    for(i = 0; i < KEY_IDX_MAX ; i++)
    {
      uint32_t idxVal[2];
      SE_LL_FLASH_OTP_Read((OTP_ENC_KEY_IDX_WORD_START+i), (uint8_t*)&idxVal[0], 1);
      if ( (idxVal[0] != 0xFFFFFFFF) || (idxVal[1] != 0xFFFFFFFF))
      {
        /* we have found the key index min value from OTP */
        enckeyidx_min = i+1;      
...
#ifdef TEST_KEY_REVOCATION_STORAGE_IN_BKPR  
    // For test purpose, we get the value from BKP register first if OTP read min index is 0
    if ( enckeyidx_min == 0 )
    {
      enckeyidx_min = (uint8_t)(TAMP->BKP1R & 0xFF);
    }
#endif    
    if (enckeyidx >= enckeyidx_min)
    {
      if ( (enckeyidx > enckeyidx_min) && (updateOTP == 1))
      {
        /* Program OTP to set the new MIN INDEX FLAG */
#ifdef TEST_KEY_REVOCATION_STORAGE_IN_BKPR  
        // For test purpose, we store the MIN INDEX value in a RAM variable first
        TAMP->BKP1R = enckeyidx;
#else 
        uint32_t idxVal[2] = {0,0};
        SE_LL_FLASH_OTP_Write((OTP_ENC_KEY_IDX_WORD_START + enckeyidx - 1), (uint8_t*)&idxVal[0], 1);
#endif        
      }
      ret = 1;
    }
...
static void SE_CRYPTO_AES_ReadKey(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
...
#else /* !(defined KEY_REVOCATION)   */  
  uint8_t keyindex = pxSE_Metadata->EncKeyIndex;
  if ( ValidateKeyIndex(pxSE_Metadata, 0x2, 0) != 1 ) 
  {
    SE_CLEAN_UP_FW_KEY();
    return;
  }
  ...
  if (memcmp(pxSE_Metadata->SFUMagic, SFUM_1, strlen(SFUM_1)) == 0)
  {
    ...
    read_key_fp[keyindex](&(m_aSE_FirmwareKey[0]));
#else
    /* change the code to use switch case instead of function pointer table
     * this is to avoid CubeIDE compile to generate BLX instruction when calling function pointer */
    switch(keyindex)
    {
    case 0:  SE_ReadKey_1_0(&(m_aSE_FirmwareKey[0])); break;
    case 1:  SE_ReadKey_1_1(&(m_aSE_FirmwareKey[0])); break;
    case 2:  SE_ReadKey_1_2(&(m_aSE_FirmwareKey[0])); break;
    case 3:  SE_ReadKey_1_3(&(m_aSE_FirmwareKey[0])); break;
    case 4:  SE_ReadKey_1_4(&(m_aSE_FirmwareKey[0])); break;
    case 5:  SE_ReadKey_1_5(&(m_aSE_FirmwareKey[0])); break;
    case 6:  SE_ReadKey_1_6(&(m_aSE_FirmwareKey[0])); break;
    case 7:  SE_ReadKey_1_7(&(m_aSE_FirmwareKey[0])); break;
    default: break;
    }
#endif
  }
...
static void SE_CRYPTO_ReadKey_Pub(SE_FwRawHeaderTypeDef *pxSE_Metadata, uint8_t *paSE_PubKey)
{
...
#else /* !(defined KEY_REVOCATION)   */
  /* In case of key revocation support, we will
     1) check the key index in the header and check key index against OTP value
     2) determin the public key function to read the key data
     3) read the key data and verify the key hash value
   */
  uint8_t keyindex = pxSE_Metadata->PubKeyIndex;
  if ( ValidateKeyIndex(pxSE_Metadata, 0x1, 0) != 1 ) 
  {    
    /* ECC public key */
    return;
  }
  /* TODO: read key index from header and check key index against OTP value */
  ...
  if (memcmp(pxSE_Metadata->SFUMagic, SFUM_1, strlen(SFUM_1)) == 0)
  {
    ...
    read_key_fp[keyindex](paSE_PubKey);
#else
    /* change the code to use switch case instead of function pointer table
     * this is to avoid CubeIDE compile to generate BLX instruction when calling function pointer */
    switch(keyindex)
    {
    case 0:  SE_ReadKey_1_Pub_0(paSE_PubKey); break;
    case 1:  SE_ReadKey_1_Pub_1(paSE_PubKey); break;
    case 2:  SE_ReadKey_1_Pub_2(paSE_PubKey); break;
    case 3:  SE_ReadKey_1_Pub_3(paSE_PubKey); break;
    case 4:  SE_ReadKey_1_Pub_4(paSE_PubKey); break;
    case 5:  SE_ReadKey_1_Pub_5(paSE_PubKey); break;
    case 6:  SE_ReadKey_1_Pub_6(paSE_PubKey); break;
    case 7:  SE_ReadKey_1_Pub_7(paSE_PubKey); break;
    default: break;
    }
#endif
  }
...
static int32_t SE_CRYPTO_SHA384_HASH_DigestCompute(const uint8_t *InputMessage, const int32_t InputMessageLength,
                                                   uint8_t *MessageDigest, int32_t *MessageDigestLength);
                                                   ...
                                                   uint8_t *MessageDigest, int32_t *MessageDigestLength)
{
 // TODO FIXME
  ...
  ret = mbedtls_sha512_starts_ret(&ctx, 1);   /* 1 for sha384 */

  if (0 == ret)
  {
    ret = mbedtls_sha512_update_ret(&ctx, InputMessage, (size_t)InputMessageLength); /* casting is fine because size_t
                                                                                        is unsigned int in ARM C */

    if (0 == ret)
    {
      ret = mbedtls_sha512_finish_ret(&ctx, MessageDigest);

      if (0 == ret)
      {
        error_status = 0;
        *MessageDigestLength = 48; /* sha384 */
      }
      else
      {
        *MessageDigestLength = 0;
...
SE_ErrorStatus SE_CRYPTO_Decrypt_Init(SE_FwRawHeaderTypeDef *pxSE_Metadata, uint32_t SE_FwType)
{
   ...
   */
#elif  SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384
      /*
       * In this crypto scheme the Firmware is not encrypted.
       * The Decrypt operation is called anyhow before installing the firmware.
       * Indeed, it allows moving the Firmware image blocks in FLASH.
...
#elif ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384))
...
#elif  ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256)\
        ||(SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384))
...
SE_ErrorStatus SE_CRYPTO_AuthenticateFW_Init(SE_FwRawHeaderTypeDef *pxSE_Metadata, uint32_t SE_FwType)
{
  ...
  mbedtls_sha512_init(&m_SHA512ctx);

  ret = mbedtls_sha512_starts_ret(&m_SHA512ctx, 1); /* is384:1 for sha384 */
  ...
  {
    mbedtls_sha512_free(&m_SHA512ctx);
  }
#else
#error "The current example does not support the selected crypto scheme."
#endif /* SECBOOT_CRYPTO_SCHEME */
                                               ...
                                               int32_t *pOutputSize)
{
  ...
  ret = mbedtls_sha512_update_ret(&m_SHA512ctx, pInputBuffer, (size_t)InputSize);
...
#elif ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384) )
  int32_t ret = mbedtls_sha512_finish_ret(&m_SHA512ctx, pOutputBuffer);

  if (0 == ret)
  {
    e_ret_status = SE_SUCCESS;
    *pOutputSize = 48; /* sha384 so 48 bytes */
  }
  else
  {
    *pOutputSize = 0;
...
SE_ErrorStatus SE_CRYPTO_Authenticate_Metadata(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
  ...
  /**
    ...
    */
static uint8_t m_aSE_PubKey[SE_ASYM_PUBKEY_LEN] = {0};
#endif /* SECBOOT_CRYPTO_SCHEME */
...
#elif ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384))
  /* buffer for sha384 computing */
  uint8_t MessageDigest[48];      /* The message digest is a sha384 so 48 bytes */
#else
#endif
#endif /* SECBOOT_CRYPTO_SCHEME */
  ...
  /* Verify the HeaderSignature with mbedTLS and the AES GCM Additional Authenticated Data */
  mbedtls_cipher_init(&(m_AESGCMctx.mbedAesGcmCtx));
  ret = mbedtls_cipher_setup(&(m_AESGCMctx.mbedAesGcmCtx), mbedtls_cipher_info_from_values(MBEDTLS_CIPHER_ID_AES,
                             SE_SYMKEY_LEN * 8, MBEDTLS_MODE_GCM));
  ...
  /**
    ...
    */
  memcpy(m_AESGCMctx.aTag, (uint8_t *)pxSE_Metadata->HeaderSignature, SE_TAG_LEN);
  m_AESGCMctx.TagSize = (size_t)SE_TAG_LEN;
  ...
  /* signature to be verified with r and s components */
  pSign_r = pSign;
  pSign_s = (uint8_t *)(pSign + 48);

  /* Compute the SHA256 of the Firmware Metadata */
  status = SE_CRYPTO_SHA384_HASH_DigestCompute(pPayload,
                                               payloadSize,
                                               (uint8_t *)MessageDigest,
                                               &MessageDigestLength);
  ...
  if (0 == status)
  {
    ...
    {
      /* We must use the mbedTLS format for the public key */
      unsigned char aKey[97];
      aKey[0] = 0x04; /* uncompressed point */
      (void)memcpy(&aKey[1], pKey, 96);

      /* Public key loaded as point Q */
      ret = mbedtls_ecp_point_read_binary(&grp, &Q, aKey, 97);
      ...
      else
      {
        /**
          ...
          * Prepare the signature to be verified (r and s components).
          */
        ret = mbedtls_mpi_read_binary(&r, pSign_r, 48);
        ret += mbedtls_mpi_read_binary(&s, pSign_s, 48);
...
#endif /* SECBOOT_CRYPTO_SCHEME */
#if defined KEY_REVOCATION
  /* If the signature validation is OK, we need to update key index value if OTP if necessary */
  #if (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384) \
      || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384)  
  if ( e_ret_status == SE_SUCCESS )
  {
    ValidateKeyIndex(pxSE_Metadata, 0x3, 1);
  }
  #endif
#endif
  /* Return status*/
