...
//#define UART_TX_RX_DELAY HAL_MAX_DELAY
/* Change delay from max delay to 200 to give chance for IWDG feed */
#define UART_TX_RX_DELAY 200

static void UART_TransmitBuffer(void)
{
    HAL_UART_Transmit (&huart2, (uint8_t*)&txtrBuffer.dataSize, 2, 10);
    HAL_UART_Transmit (&huart2, txtrBuffer.buffer, txtrBuffer.dataSize, HAL_MAX_DELAY);
}

static void UART_ReceiveBuffer(void)
{
    uint16_t size;
    while ( HAL_UART_Receive( &huart2, (uint8_t*)&size, 2, UART_TX_RX_DELAY ) != HAL_OK ) 
    {
      /* If the SecureBoot configured the IWDG, UserApp must reload IWDG counter 
          with value defined in the reload register */
      WRITE_REG(IWDG->KR, IWDG_KEY_RELOAD);
    };
    while (HAL_UART_Receive( &huart2, txtrBuffer.buffer, size, UART_TX_RX_DELAY ) != HAL_OK ) 
    {
      /* If the SecureBoot configured the IWDG, UserApp must reload IWDG counter 
          with value defined in the reload register */
      WRITE_REG(IWDG->KR, IWDG_KEY_RELOAD);
    };
    txtrBuffer.dataSize = size;
}

extern  uint8_t test_QueryCurveType(void);

static void spdm_demo(void)
{
  uint8_t status_code;
  ...
  //required to use stm32 cryptolibs
  __CRC_CLK_ENABLE();
...
//#endif  
  ...
  {
    /* reconfigure IWDG to allow a longer time for feeding */
    IWDG_HandleTypeDef   IwdgHandle;
    IwdgHandle.Instance = IWDG;
    IwdgHandle.Init.Prescaler = IWDG_PRESCALER_64;
    IwdgHandle.Init.Reload = (20 * LSI_VALUE / 64U);
    IwdgHandle.Init.Window = IWDG_WINDOW_DISABLE;
  
    HAL_IWDG_Init(&IwdgHandle) == HAL_OK;
  }
  ...
  printf("\r\n=              Build with KEIL %s %s                  =", __DATE__, __TIME__);
#elif defined (__ICCARM__) 
  printf("\r\n=              Build with IAR %s %s                   =", __DATE__, __TIME__);
#elif defined(__GNUC__)	
  printf("\r\n=              Build with GCC %s %s                   =", __DATE__, __TIME__);
...
#if defined SPDM_DEMO
        case '6' :
          printf("Start SPDM demo, please disconnect com from console tool on host PC\r\n");  
          COM_DeInit();
          spdm_demo();
          break;          
#endif            
