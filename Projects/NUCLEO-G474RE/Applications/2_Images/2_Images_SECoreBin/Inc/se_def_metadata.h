...
typedef struct
{
  ...
  uint8_t  Reserved[2U];          /*!< Reserved for future use: 28 extra bytes to have a header size of 232 bytes ==> To make the header size aligned to flash write size (8bytes) */
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
...
typedef struct
{
  ...
  uint8_t  EncKeyIndex;            /*!< Index of the key used for FW encryption. In case key revocation not supported, this field is always 0 */
  uint8_t  PubKeyIndex;            /*!< Index of the pub key used for FW verification. In case key revocation not supported, this field is always 0 */
  uint8_t  Reserved[18U];          /*!< Reserved for future use: 44 extra bytes to have a header size of 232 bytes ==> To make the header size aligned to flash write size (8bytes)*/
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
