...
magic="SFU"$fwid
oemkey=$SecureEngine"/../Binary/OEM_KEY_COMPANY"$fwid"_key_AES256_CBC.bin"
ecckey=$SecureEngine"/../Binary/ECCKEY"$fwid"-384.txt"
...
command=$cmd" "$prepareimage" enc -k "$oemkey"  --enckeyidx "$enckeyidx" --keynum "$keynum" -i "$iv" "$bin" "$sfu
$command > "$projectdir"/output.txt
ret=$?
if [ $ret -eq 0 ]; then
  command=$cmd" "$prepareimage" sha384 "$bin" "$sign
  $command >> $projectdir"/output.txt"
  ret=$?
  if [ $ret -eq 0 ]; then 
    command=$cmd" "$prepareimage" pack -m "$magic" -k "$ecckey" --enckeyidx "$enckeyidx" --signkeyidx "$signkeyidx" --keynum "$keynum" -r 2 -v "$version" -i "$iv" -f "$sfu" -t "$sign" "$sfb" -o "$offset
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret -eq 0 ]; then
      command=$cmd" "$prepareimage" header -m "$magic" -k  "$ecckey" --enckeyidx "$enckeyidx" --signkeyidx "$signkeyidx" --keynum "$keynum" -r 2 -v "$version"  -i "$iv" -f "$sfu" -t "$sign" -o "$offset" "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret -eq 0 ]; then
        ...
        if [ $ret -eq 0 ] && [ -e "$ref_userapp" ]; then
          ...
          ret=$?
          if [ $ret -eq 0 ]; then
            command=$cmd" "$prepareimage" enc -k "$oemkey" --enckeyidx "$enckeyidx" -i "$iv" "$partialbin" "$partialsfu
            $command >> $projectdir"/output.txt"
            ret=$?
            if [ $ret -eq 0 ]; then
              command=$cmd" "$prepareimage" sha384 "$partialbin" "$partialsign
              $command >> $projectdir"/output.txt"
              ret=$?
              if [ $ret -eq 0 ]; then
                command=$cmd" "$prepareimage" pack -m "$magic" -k "$ecckey" --enckeyidx "$enckeyidx" --signkeyidx "$signkeyidx" --keynum "$keynum" -r 2 -v "$version" -i "$iv" -f "$sfu" -t "$sign" -o "$offset" --pfw "$partialsfu" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
                $command >> $projectdir"/output.txt"
                ret=$?
              fi
            fi
