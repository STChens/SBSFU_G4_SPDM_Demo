...
if [ $ret -eq 0 ]; then
  ...
  if [ "$crypto" = "SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384" ]; then
    type="CBC"
    aeskeylen=256
  fi

  if [ $type != "vide" ]; then    
    oemkey="$1/../Binary/OEM_KEY_COMPANY1_key_AES${aeskeylen}_${type}.bin"
    command=$cmd" "$prepareimage" trans -a GNU -k "$oemkey" -f SE_ReadKey_1 -v "$cortex" -n "$keynum
    echo $command
    $command >> $asmfile
    ret=$?
        
    if [ $ret -eq 0 ]; then
      oemkey="$1/../Binary/OEM_KEY_COMPANY2_key_AES${aeskeylen}_${type}.bin"
      if [ -e "$oemkey" ]; then
        command=$cmd" "$prepareimage" trans -a GNU -k "$oemkey" -f SE_ReadKey_2 -v "$cortex" -n "$keynum
        echo $command
        $command >> $asmfile
        ret=$?
      fi
    ...
    if [ $ret -eq 0 ]; then
        oemkey="$1/../Binary/OEM_KEY_COMPANY3_key_AES${aeskeylen}_${type}.bin"
        if [ -e "$oemkey" ]; then
            command=$cmd" "$prepareimage" trans -a GNU -k "$oemkey" -f SE_ReadKey_3 -v "$cortex" -n "$keynum
            echo $command
            $command >> $asmfile
            ret=$?
        fi
...
if [ $ret -eq 0 ]; then
  ...
  if [ "$crypto" = "SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384" ]; then
    type="ECC"
    ecckeylen="-384"
  fi  
  if [ "$crypto" = "SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384" ]; then
  ...
  if [ $type != "vide" ]; then
    ecckey=$1"/../Binary/${pubprefix}ECCKEY1${ecckeylen}.txt"
    echo "ecc key file to be translated:" $ecckey >> $1/output.txt 
    command=$cmd" "$prepareimage" trans  -a GNU -k "$ecckey" -f SE_ReadKey_1_Pub -v "$cortex" -n "$keynum
    echo $command
    $command >> $asmfile
    ret=$?

    if [ $ret -eq 0 ]; then
      ecckey=$1"/../Binary/${pubprefix}ECCKEY2${ecckeylen}.txt"
      if [ -e "$ecckey" ]; then
        command=$cmd" "$prepareimage" trans  -a GNU -k "$ecckey" -f SE_ReadKey_2_Pub -v "$cortex" -n "$keynum
        echo $command
        $command >> $asmfile
        ret=$?
      fi
    ...
    if [ $ret -eq 0 ]; then
      ecckey=$1"/../Binary/${pubprefix}ECCKEY3${ecckeylen}.txt"
      if [ -e "$ecckey" ]; then
        command=$cmd" "$prepareimage" trans  -a GNU -k "$ecckey" -f SE_ReadKey_3_Pub -v "$cortex" -n "$keynum
        echo $command
        $command >> $asmfile
        ret=$?
      fi
