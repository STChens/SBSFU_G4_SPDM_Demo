...
/** @defgroup SE_CRYPTO_BOOTLOADER_Private_Variables_SHA256 SHA256 variables
  ...
  *  @{
  */
static SHA384ctx_stt m_SHA384ctx; /*!< Variable used to store the SHA256 context (this one does not really need to be
                                       protected as it is a HASH) */
/**
  * @}
  */
...
#define SE_CLEAN_UP_PUB_KEY() \
    do { \
      (void)memcpy(m_aSE_PubKey, (void const *)(SE_STARTUP_REGION_ROM_START + (SysTick->VAL % 0xFFFU)), \
                   sizeof(m_aSE_PubKey)); \
      (void)memcpy((void *)&m_SHA384ctx, (void const *)(SE_STARTUP_REGION_ROM_START + (SysTick->VAL % 0xFFFU)), \
                   sizeof(m_SHA384ctx)); \
    } while(0)
...
#elif (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES256_CBC_SHA384)
  /*
   * No need for a local struct like se_gcm_init because we do not rely on the SE common crypto code (as AES CBC is not
   * available for the UserApp).
   * We call directly the Cryptolib.
...
SE_ErrorStatus SE_CRYPTO_AuthenticateFW_Init(SE_FwRawHeaderTypeDef *pxSE_Metadata, uint32_t SE_FwType)
{
  ...
  else
  {
    ...
    /* Set the size of the desired hash digest: SHA-256 */
    m_SHA384ctx.mTagSize = CRL_SHA384_SIZE;
  
    /* Set flag field to default value */
    m_SHA384ctx.mFlags = E_HASH_DEFAULT;
  
    /* Initialize the HASH context */
    error_status = SHA384_Init(&m_SHA384ctx);
                                               ...
                                               int32_t *pOutputSize)
{
  ...
  else
  {
    ...
    /* Add data to be hashed */
    error_status = SHA384_Append(&m_SHA384ctx, pInputBuffer, InputSize);
...
SE_ErrorStatus SE_CRYPTO_AuthenticateFW_Finish(uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  ...
  else
  {
    ...
    /* Add data to be hashed */
    error_status = SHA384_Finish(&m_SHA384ctx, pOutputBuffer, pOutputSize);
...
SE_ErrorStatus SE_CRYPTO_Authenticate_Metadata(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
  ...
  /******************************************************************************/
  /******** Parameters for Elliptic Curve P-384 SHA-384 from FIPS 186-4**********/
  /******************************************************************************/
  static const uint8_t P_384_a[]  __attribute__((aligned(4))) =
    {
  ...
  /* coefficient b */
  static const uint8_t P_384_b[]  __attribute__((aligned(4))) =
    {
  ...
  /* prime modulus p*/
  static const uint8_t P_384_p[]  __attribute__((aligned(4))) =
    {
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00,
  ...
  /* order n*/
  static const uint8_t P_384_n[]  __attribute__((aligned(4))) =
    {
      0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
  ...
  /* base point Gx*/
  static const uint8_t P_384_Gx[]  __attribute__((aligned(4))) =
    {
  ...
  /* base point Gy*/
  static const uint8_t P_384_Gy[]  __attribute__((aligned(4))) =
    {
  ...
  /** pre-allocated memory buffer used to store the internal values required by ECCDSA computation */
  static uint8_t preallocated_buffer[3584];
#endif /* SECBOOT_CRYPTO_SCHEME */
  ...
  if (status == HASH_SUCCESS)
  {
    ...
    {
      /* Point is initialized, now import the public key */
      (void)ECCsetPointCoordinate(PubKey, E_ECC_POINT_COORDINATE_X, pPub_x, 48);
      (void)ECCsetPointCoordinate(PubKey, E_ECC_POINT_COORDINATE_Y, pPub_y, 48);
      /* Try to validate the Public Key. */
      status = ECCvalidatePubKey(PubKey, &EC_st, &Crypto_Buffer);
    }
    ...
    {
      /* Import the signature values */
      (void)ECDSAsetSignature(sign, E_ECDSA_SIGNATURE_R_VALUE, pSign_r, 48);
      (void)ECDSAsetSignature(sign, E_ECDSA_SIGNATURE_S_VALUE, pSign_s, 48);
