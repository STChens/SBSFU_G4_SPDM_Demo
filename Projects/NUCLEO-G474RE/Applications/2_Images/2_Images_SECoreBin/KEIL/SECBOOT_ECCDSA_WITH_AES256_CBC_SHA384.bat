::Post build for SECBOOT_ECCDSA_WITH_AES128_CBC_SHA384
...
set "sbsfuelf=%SBSFUBootLoader%\\2_Images_SBSFU\\KEIL\\NUCLEO-G474RE\\Exe\\SBSFU.axf"
set "magic=SFU%fwid%"
set "offset=4096"
set "alignment=16"
...
set "command=%python%%prepareimage% enc -k %oemkey% --enckeyidx %enckeyidx% --keynum %keynum% -i %iv% %bin% %sfu%  > %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% sha384 %bin% %sign% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% pack -m %magic% -k %ecckey% --enckeyidx %enckeyidx% --signkeyidx %signkeyidx% --keynum %keynum% -r 2 -v %version% -i %iv% -f %sfu% -t %sign% %sfb% -o %offset% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% header -m %magic% -k  %ecckey% --enckeyidx %enckeyidx% --signkeyidx %signkeyidx% --keynum %keynum% -r 2 -v %version%  -i %iv% -f %sfu% -t %sign% -o %offset% %headerbin% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
...
set "command=%python%%prepareimage% enc -k %oemkey% --enckeyidx %enckeyidx% --keynum %keynum% -i %iv% %partialbin% %partialsfu%  >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% sha384  %partialbin% %partialsign% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% pack -m %magic% -k %ecckey% --enckeyidx %enckeyidx% --signkeyidx %signkeyidx% --keynum %keynum% -r 2 -v %version% -i %iv% -f %sfu% -t %sign% -o %offset% --pfw %partialsfu% --ptag %partialsign% --poffset  %partialoffset% %partialsfb%>> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
