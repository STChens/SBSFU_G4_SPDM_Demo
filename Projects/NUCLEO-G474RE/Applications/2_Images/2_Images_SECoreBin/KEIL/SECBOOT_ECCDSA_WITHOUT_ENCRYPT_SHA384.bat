::Post build for SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA384
...
set "command=%python%%prepareimage% pack -m %magic% -k %ecckey% -p 1 --signkeyidx %signkeyidx% --keynum %keynum% -r 18 -v %version% -f %bin% -t %sign% %sfb% -o %offset% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% header -m %magic% -k %ecckey% -p 1 --signkeyidx %signkeyidx% --keynum %keynum% -r 18 -v %version% -f %bin% -t %sign% -o %offset% %headerbin% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
...
set "command=%python%%prepareimage% pack -m %magic% -k %ecckey% -p 1 --signkeyidx %signkeyidx% --keynum %keynum% -r 18 -v %version% -f %bin% -t %sign% -o %offset% --pfw %partialbin% --ptag %partialsign% --poffset  %partialoffset% %partialsfb%>> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
